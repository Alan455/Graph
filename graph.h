...
/* Crea un nuovo grafo con `n` nodi. 
   Il numero di nodi deve essere strettamente positivo. 
   Il tipo di grafo (orientato oppure non orientato) è specificato dal parametro `t`. */
...
/* Aggiunge un nuovo arco (src, dst) con peso "weight". 
   Si puo' assumere che l'arco non esista già (si può omettere il controllo, anche se nella soluzione fornita viene fatto).
   Nel caso di grafo non orientato, occorre aggiungere l'arco sia nella lista di adiacenza di `src` che in quella di `dst`. */
