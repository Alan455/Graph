...
typedef struct Edge {
    ...
    double weight;
    struct Edge *next;
} Edge;      `Edge` rappresenta un arco del grafo
...
Edge: (arco del grafo)
Per grafi non orientati,ogni arco $(u,v)$ deve essere presente due volte: 
una come $(u,v)$ nella lista di adiacenza di $u$,
una come $(v,u)$ nella lista di adiacenza di $v$. 
Le liste sono concatenate semplici; il campo `next` indica l'arco successivo della lista di adiacenza, oppure `NULL` se è l'ultimo nodo.

La struttura `Graph` rappresenta l'intero grafo; la spiegazione dei
vari campi è nel file [graph.h](graph.h). 
edges` è un array di lunghezza $n$ di puntatori: 
edges[v]` punta all'inizio della lista di adiacenza del nodo $v$, oppure `NULL` se $v$ non ha archi uscenti. 
in_deg` e `out_deg` sono array di lunghezza $n$; 
in_deg[v] e out_deg[v]` sono, rispettivamente, il grado entrante e il grado uscente di $v$. 
Nel caso di grafi non orientati, si deve avere `out_deg[v] == in_deg[v] per ogni $v$, 
dato che ogni arco viene considerato sia entrante che uscente su entrambi gli estremi. 
È necessario mantenere l'informazione sui gradi entranti/uscenti durante la costruzione del grafo.
...
/*
    ...
    Il comportamento di questa funzione sarà leggermente diverso a seconda che il grafo sia orientato oppure no.
*/
void graph_add_edge(Graph *g, int src, int dst, double weight){
    ...
    //L'arco (src,dst,weight) viene inserito nella lista di adiacenza del nodo `src`;
    ...
    //L'arco non orientato (src,dst,weight) viene inserito sia nella lista di adiacenza del nodo `src`, che in quella del nodo `dst`
    newNode->next = NULL;
    newNode->weight = weight;
    newNode->src = dst;
    newNode->dst = src;
    newNode->next = g->edges[dst]->next;
    g->edges[dst] = newNode;
...
void graph_print(const Graph *g){
    ...
    for (;nodeIndex < g->n; nodeIndex++) {
        tmp = graph_adj(g, nodeIndex);
        assert(tmp != NULL);
        while (tmp->next != NULL) {
            if (tmp->dst == 9) {
                tmp = g->edges[i];
